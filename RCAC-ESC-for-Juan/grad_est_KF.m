classdef grad_est_KF
    ...
    properties
        ...
        Q_KF;       % Kalman filter Q matrix.
        R_KF;       % Kalman filter R matrix.
        ii_KF;      % Number of estimator steps.
    end
    
    methods
        function obj = grad_est_KF(lu, lz, indi, P0, Q, R)
            % Initialization
            obj.lu = lu;
            obj.lz = lz;
        ...
        function [obj, grad_est]= oneStep(obj, u, J, en)
            ...
            if en > 0.5
                if obj.ii_KF >= obj.d_max
                    ...
                    Hk = [u_vec ones(1+obj.lu,1)];
                    for ii = 1:obj.lz
                        Gk = (obj.J_buffer(ii,obj.ind_vec)).';
                        obj.P_KF(:,:,ii) = obj.P_KF(:,:,ii) + obj.Q_KF(:,:,ii);
                        KK = (obj.P_KF(:,:,ii)*Hk')/(Hk*obj.P_KF(:,:,ii)*Hk' + obj.R_KF(:,:,ii));
                        obj.dJ_KF(:,ii) = obj.dJ_KF(:,ii) + KK*(Gk - Hk*obj.dJ_KF(:,ii));
                        obj.P_KF(:,:,ii) = (eye(1+obj.lu) - KK*Hk)*obj.P_KF(:,:,ii);
                    end
                else
                    obj.ii_KF = obj.ii_KF + 1.0;
                end
            end
